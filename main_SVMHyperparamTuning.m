%% SVR Hyperparameters optimization
...
figure()
hold on;
legend('location', 'best', 'Box','off', 'FontSize',18 );
xlabel('N° of cycle', "FontSize",18) ;
ylabel('SoH', "FontSize",18);
title ('SVM fitting B0005', "FontSize",18);
plot ([1:length(Y_5)], Y_5,'b', 'DisplayName', "Measured SoH", "LineWidth",2);
hold on
yline(0.8, '-', 'color','r', 'LineWidth',1.5, "DisplayName","EoL");
%}
...
rng default
%model_3 = fitrsvm(X_train,Y_train,  "OptimizeHyperparameters",'auto', "HyperparameterOptimizationOptions", struct(MaxObjectiveEvaluations=30, ShowPlots=false));
...
%model = fitrsvm(X_train,Y_train, Standardize=model_3.ModelParameters.StandardizeData, BoxConstraint = model_3.ModelParameters.BoxConstraint, Epsilon=model_3.ModelParameters.Epsilon, KernelScale=model_3.ModelParameters.KernelScale);
